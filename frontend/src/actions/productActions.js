...
import {
  ...
  PRODUCT_LIST_POPULAR_FAIL,
  PRODUCT_LIST_POPULAR_REQUEST,
  PRODUCT_LIST_POPULAR_SUCCESS,
...
export const listPopularProducts = () => async (dispatch) => {
  dispatch({
    type: PRODUCT_LIST_POPULAR_REQUEST,
  });
  try {
    const { data } = await Axios.get(
      `/api/products?order=popular&pagesize=8`
    );
    dispatch({ type: PRODUCT_LIST_POPULAR_SUCCESS, payload: data });
  } catch (error) {
    dispatch({ type: PRODUCT_LIST_POPULAR_FAIL, payload: error.message });
  }
};

export const listTopRatedProducts = () => async (dispatch) => {
  dispatch({
    type: PRODUCT_LIST_TOPRATED_REQUEST,
  });
  try {
    const { data } = await Axios.get(
      `/api/products?order=toprated&pagesize=4`
    );
    dispatch({ type: PRODUCT_LIST_TOPRATED_SUCCESS, payload: data });
  } catch (error) {
    dispatch({ type: PRODUCT_LIST_TOPRATED_FAIL, payload: error.message });
  }
};
