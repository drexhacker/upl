    ...
    expressAsyncHandler(async (req, res) => {
        ...
        try {
            ...
            const payload = {
                ...
                "redirect_url": "https://xifair.shop/order/" + req.params.id,
                "network": "Airtel"
                // "enc_key": data.encKey
            }
...
orderRouter.post(
    ...
    expressAsyncHandler(async (req, res) => {
        if (req.body.orderItems.length === 0) {
            ...
            res.send({ message: 'Order Placed Suuccefully', order: createdOrder });
        }
    })
);
...
orderRouter.put(
    ...
    expressAsyncHandler(async (req, res) => {
        ...
        if (order) {
            ...
            ejs.renderFile(path.join(__dirname, './views/', "invoice-template.ejs"), { order: updatedOrder, invoiceId: invoiceId, number: '256708932049' }, (err, data) => {
                if (err) {
                    res.send(err);
                } else {
                    let options = {
                    ...
                    pdf.create(data, options).toFile("frontend/temps/Invoice" + updatedOrder._id + ".pdf", async (err, data) => {
                        if (err) {
                            ...
                            try {
                                const transporter = nodemailer.createTransport({
                                    ...
                                    secure: false,
                                    auth: {
                                        user: "xifair@drexsoft.xyz",
                                        pass: "Password@2020",
                                    },
                                });
                                console.log(transporter)
                                const generatedForm = "https://xifair.shop/api/files?filename=Invoice" + createdOrder._id + ".pdf";
                                const bodyMessage = 'Hello ' + req.user.name + ',\nCongratulations, Your Order has been placed successfully. An email has been sent to containing your invoice, download it and use it order tracking and order pickup. \n Thank You for using Our services!\n\nRegards XiFair Shoppers Team';
                                const info = await transporter.sendMail({
                                    from: '"XiFair Shoppers Limited 🛍️" <xifair@drexsoft.xyz>', // sender address
                                    to: "admin@drexsoft.xyz, derrickmubiru307@gmail.com, " + req.user.email, // list of receivers
                                    subject: "XIFAIR SHOPPERS INVOICE", // Subject line
                                    text: bodyMessage, // html body
                                    attachments: [
                                        {
                                            "filename": "XiFair Invoice.pdf",
                                            "path": generatedForm
                                        }
                                    ],
                                }, (error, result) => {
